...
/**
 * The builder's Assign tab (formerly AssignDialog): current assignments plus
 * the new-assignment form. Mounted fresh each time the tab activates, so the
 * data loads on mount.
 */
const t = useT()
  ...
  <div v-else class="mx-auto max-w-2xl space-y-5">
      ...
      <ul v-else class="space-y-1">
        ...
        >
          <Badge variant="secondary">
            {{
              assignment.classroomId ? t.staff.assign.classroomBadge : t.staff.assign.studentBadge
            }}
          </Badge>
    ...
    <div v-if="canAssign" class="space-y-3 rounded-lg border p-3">
      ...
      <Field>
        ...
        <Input id="assign-due" v-model="dueAtLocal" type="datetime-local" :disabled="isSaving" />
      </Field>
